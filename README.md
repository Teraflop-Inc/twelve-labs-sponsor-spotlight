...
Sponsorship analytics over broadcast footage, built on **TwelveLabs Jockey**.
Point it at an indexed broadcast and it discovers the sponsors present, times
every appearance, scores how legibly each mark renders on air, and turns that
into media value traceable back to a timestamp.

A walkthrough of the four analysis passes is in
[docs/blog/sponsor-intelligence-with-jockey.md](docs/blog/sponsor-intelligence-with-jockey.md).

---

## What it does

- **Brand discovery** — one Jockey call enumerates every sponsor visible in the
  footage, including small, partial and briefly-visible marks. No watchlist to
  maintain.
- **Timed exposure** — per-brand appearances with start/end seconds, surface
  (perimeter LED, shirt-front, backdrop, broadcast overlay…), camera framing,
  placement, match period, and the game clock read from the scorebug.
- **Match-event context** — dedicated passes locate every goal, celebration and
  replay, then stamp them onto overlapping sponsor moments by interval overlap.
- **Legibility audit** — each brand asset scored 0–10 on contrast, size,
  position, camera angle and motion blur, with timestamped examples and
  suggested creative fixes.
- **Media value** — EMV from seconds × legibility × clutter × audience-at-minute
  × rate, computed client-side. Each economic input is labelled
  Customer-Uploaded, Simulated, or Manual.
- **Highlight reels** — a ~60s cut of a brand's top moments per game, built
  offline with FFmpeg over the broadcast HLS and served from Vercel Blob.
- **Export and reporting** — CSV/JSON per game plus aggregate totals, and a
  printable performance report whose figures match the screen.
...
| 2 | **Economic assumptions** | Rights fee, plus optional audience-curve and rate-card uploads; every $ figure recomputes live and is labelled by source. |
| 3 | **Brand discovery** | One Jockey call lists every sponsor in the footage; select which to analyze. A live Analyze run covers two at a time (`MAX_LIVE_BRANDS`); the offline capture script does more (`SPONSOR_SPOTLIGHT_ANALYZE_TOPN`, default 8). |
...
The split exists because a Jockey `/responses` call reasons over the whole
broadcast and takes ~45s–3min — too slow to sit behind a click on the demo path.
...
```bash
export TWELVELABS_API_KEY=tlk_...                                # export it directly
TWELVELABS_API_KEY=tlk_... uv run uvicorn main:app --port 8001   # or inline
```

> Jockey (`jockey1.0`) is in beta. Check the TwelveLabs docs for current access
> requirements and request limits.
...
at one call per 30s (`SPONSOR_SPOTLIGHT_CAPTURE_DELAY`) to stay well inside
whatever request limit applies to your key.
...
- **Rate limits.** Discovery is one call; analysis is capped at two brands (two
  calls). The backend retries `429`/`5xx` with backoff that honors the
  rate-limit reset time in the response, so it adapts to whatever limit your key

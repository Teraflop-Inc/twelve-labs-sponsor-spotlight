...
import type { Moment, Provenance } from "./lib/types"
...
/**
 * Says whether a result was computed live or replayed from a committed fixture.
 *
 * The server decides this and stamps it on the response — see
 * `backend/domain/sponsor/provenance.py`. Rendering what we're told (rather
 * than inferring from the mode, as this UI used to) means the badge stays
 * correct in the cases where a demo request silently runs live anyway: a brand
 * missing from the fixture, or a fixture captured for a different knowledge
 * store.
 */
export function ProvenanceBadge({ prov }: { prov?: Provenance | null }) {
  if (!prov) return null
  const saved = prov.from_cache
  const when = saved ? prov.captured_at : prov.generated_at
  const stamp = when ? new Date(when).toLocaleDateString() : null
  return (
    <span
      className={cn(
        "inline-flex h-5 shrink-0 items-center rounded-[6px] px-1.5 font-tl-mono text-[10px] uppercase leading-4 tracking-wide",
        saved
          ? "bg-tl-analyze-lightest-orange text-tl-analyze-dark-orange"
          : "bg-tl-embed-lightest-green text-tl-embed-dark-green",
      )}
      title={
        saved
          ? `Results from a previous analysis${stamp ? `, ${stamp}` : ""}. Re-run to generate current results.`
          : "Generated by this session's analysis."
      }
    >
      {saved ? "Saved" : "Live"}
    </span>

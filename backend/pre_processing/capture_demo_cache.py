...
Analyze runs **one brand per Jockey call, sequentially, paced** (see
``SPONSOR_SPOTLIGHT_CAPTURE_DELAY``) and writes the fixture **incrementally** —
so a run that dies on a rate limit can be re-invoked and skips brands already
captured. Legibility and
...
# One call per brand, so analyzing every discovered brand is a large batch that
# can run into a daily quota; top-N keeps a whole per-game capture manageable.

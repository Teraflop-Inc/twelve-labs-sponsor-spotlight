...
from domain.sponsor.schemas import GAME_EVENTS_SCHEMA
...
async def fetch_brand_appearances(
    ...
    try:
        resp = await jockey.responses(
            ...
            json_schema=schemas.per_brand_schema(sport),
...
async def run(
    ...
    One call per brand. The UI caps a live run at 2 brands (see MAX_LIVE_BRANDS
    in AnalyzePanel.tsx); the offline capture script analyzes more.
    """
    t0 = time.time()

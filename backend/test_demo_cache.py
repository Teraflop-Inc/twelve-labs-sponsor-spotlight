...
# --- surface vocabulary follows the sport ---------------------------------------


def test_asset_types_are_per_sport():
    """The prompt names soccer surfaces; the schema enum must permit them.

    A hardcoded basketball enum on a soccer broadcast forced every perimeter
    board to be reported as `courtside_led` and pushed the rest into `other`.
    """
    from domain.sponsor import schemas

    soccer = schemas.asset_types_for("soccer")
    basketball = schemas.asset_types_for("basketball")

    assert "perimeter_led" in soccer and "shirt_front" in soccer
    assert "courtside_led" not in soccer and "backboard" not in soccer
    assert "courtside_led" in basketball and "perimeter_led" not in basketball


def test_schema_enum_matches_the_requested_sport():
    from domain.sponsor import schemas

    def enum_of(schema):
        return schema["schema"]["properties"]["appearances"]["items"]["properties"][
            "asset_type"
        ]["enum"]

    assert "shirt_front" in enum_of(schemas.per_brand_schema("soccer"))
    assert "backboard" in enum_of(schemas.per_brand_schema("basketball"))
    # The module constant is never mutated by a build.
    schemas.per_brand_schema("basketball")
    assert "shirt_front" in enum_of(schemas.PER_BRAND_SCHEMA)


def test_every_sport_profile_declares_its_surfaces():
    import sports

    for key, profile in sports.SPORT_PROFILES.items():
        types = profile["asset_types"]
        assert types, f"{key} has no asset_types"
        assert "other" in types, f"{key} needs an 'other' escape hatch"
        assert len(set(types)) == len(types), f"{key} has duplicates"


def test_no_sport_specific_vocabulary_leaks_into_shared_schemas():
    """Schema *descriptions* are instructions too. A basketball example in a
    shared description sent the model basketball values on soccer footage."""
    import inspect
    from domain.sponsor import schemas

    src = inspect.getsource(schemas)
    body = src.split("ASSET_TYPES = ")[1]  # skip the explanatory comment block
    for term in ("courtside_led", "jersey_patch", "scorers_table", "backboard", "floor_decal"):
        assert term not in body, f"{term} leaked into a shared schema description"
